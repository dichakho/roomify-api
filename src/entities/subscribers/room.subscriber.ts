  ...
  async afterUpdate(event: InsertEvent<Room>): Promise<void> {
    ...
    property.rooms.forEach(room => {
      if(room.id === event.entity.id) {
        property.averagePrice = (property.averagePrice * count + event.entity.price - room.price) / count;
        property.averageArea = (property.averageArea * count + event.entity.area - room.area) / count;
      }
    });
