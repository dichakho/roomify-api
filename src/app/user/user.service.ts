...
export class UserService extends BaseService<User, UserRepository> {
  ...
  constructor(protected repository: UserRepository, private readonly userPermissionRepo: UserPermissionRepository) {
    super(repository);
  }
  ...
  async getPermissionOfUser(userId: number, metadata: GetMany) {
    ...
    const userData = await this.repository.findOne({ where: { id: userId }, relations: ['roles', 'roles.permissions', 'userPermissions'] });
    ...
    const query = createQueryBuilder().select('permissions').from(Permission, 'permissions');
    if (offset >= userData.userPermissions.length) {
      const permissionData = await query.leftJoin('permissions.roles', 'role').leftJoinAndSelect('permissions.module', 'module').leftJoinAndSelect('permissions.method', 'method')
        .leftJoin('role.users', 'user').where('user.id = :id', { id: userId }).take(limit).offset(offset - userData.userPermissions.length).getMany();
      data = permissionData;
    }
    else {
      const userPermissionData = await query.leftJoin('permissions.userPermissions', 'userPermission')
        .leftJoin('userPermission.user', 'user').leftJoinAndSelect('permissions.module', 'module')
        .leftJoinAndSelect('permissions.method', 'method').where('user.id= :id', { id: userId })
        .where('userPermission.status= :status', { status: 'ADD' }).take(limit).skip(offset).getMany();

      if (userPermissionData.length > 0 && limit > userPermissionData.length) {
        const permissionData = await createQueryBuilder().select('permissions')
          .from(Permission, 'permissions').leftJoin('permissions.roles', 'role').leftJoinAndSelect('permissions.module', 'module').leftJoinAndSelect('permissions.method', 'method')
          .leftJoin('role.users', 'user').where('user.id = :id', { id: userId }).take(limit - userPermissionData.length).getMany();
